...
void FtLegMed::OnStartup(Entity* self) {
...
Entity* FtLegMed::GetStandObj(Entity* self) {
...
void FtLegMed::OnSkillEventFired(Entity* self, Entity* caster, const std::string& message) {
	...
	self->AddTimer("killDrill", 0.2f);
	TriggerDrill(self);
	PlayCinematic(self);
}

void FtLegMed::TriggerDrill(Entity* self) {
...
//	movingPlatformComponent->GotoWaypoint(0);
	...
	//Attempt to play cinematic at different stage
...
void FtLegMed::OnWaypointReached(Entity* self, uint32_t waypointIndex) {
	if (waypointIndex == 1) {
...
void FtLegMed::OnUse(Entity* self, Entity* user) {
	self->Smash(self->GetObjectID());
	self->AddTimer("killDrill", 39999.2f);
	if (self->GetNetworkVar<bool>(u"bIsInUse")) {
		return;
	}
	auto* standObj = GetStandObj(self);
	RenderComponent::PlayAnimation(self, u"idle-down");	
	self->SetNetworkVar(u"bIsInUse", true);
...
void FtLegMed::FreezePlayer(Entity* self, Entity* player, bool bFreeze) {
...
void FtLegMed::OnArrived(Entity* self, uint32_t waypointIndex) {
	auto* standObj = GetStandObj(self);
...
void FtLegMed::PlayCinematic(Entity* self) {
	auto* player = Game::entityManager->GetEntity(self->GetVar<LWOOBJID>(u"userID"));
...
void FtLegMed::PlayAnim(Entity* self, Entity* player, const std::string& animName) {
...
void FtLegMed::OnHitOrHealResult(Entity* self, Entity* attacker, int32_t damage) {
...
void FtLegMed::OnTimerDone(Entity* self, std::string timerName) {
