...
//		Fetch relevant entities		
		...
		for (auto* entity : scientistEntities) {
		...
		for (auto* entity : beetleEntities) {
			auto* CombatAI = entity->GetComponent<BaseCombatAIComponent>();
			if (CombatAI != nullptr) {
				CombatAI->SetDisabled(true);
			}
		...
		for (auto* entity : bonezaiEntities) {
			auto* CombatAI = entity->GetComponent<BaseCombatAIComponent>();
			if (CombatAI != nullptr) {
				CombatAI->SetDisabled(true);
			}
		...
		for (auto* entity : overseerEntities) {
			auto* CombatAI = entity->GetComponent<BaseCombatAIComponent>();
			if (CombatAI != nullptr) {
				CombatAI->SetDisabled(true);
			}
		...
		for (auto* entity : marksmanEntities) {
			auto* CombatAI = entity->GetComponent<BaseCombatAIComponent>();
			if (CombatAI != nullptr) {
				CombatAI->SetDisabled(true);
			}
		...
		for (auto* entity : blacksmithEntities) {
			auto* CombatAI = entity->GetComponent<BaseCombatAIComponent>();
			if (CombatAI != nullptr) {
				CombatAI->SetDisabled(true);
			}
