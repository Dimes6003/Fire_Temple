...
//	all the raw code got funneled into the timers
...
//	Cinematic stuff		
    ...
    for (Entity* engaged : engagedEntities) {
        if (engaged != nullptr && proximityMonitorComponent->IsInProximity("Room_Radius", engaged->GetObjectID())
			...
			if (LargeGroup == 0) {
				GameMessages::SendPlayCinematic(engaged->GetObjectID(), u"Waves_Wave3Cam_5", 
				engaged->GetSystemAddress());	
...
//			Solo player in cinematic so screw spawner timings			
			self->AddTimer("SmallFinalSpinnersUp", 2);
			self->AddTimer("FinalEnemies", 2);		
//			Until cam moves back
			self->AddTimer("SpinnersDownChain1", 8);

			self->AddTimer("UnstunEnemies", 13);			
		} else if (LargeGroup == 1) {
			
			self->AddTimer("FinalSpinnersChain1", 6.8f);
			
			self->AddTimer("GoldSpinnerMove", 16.3f);
			
			self->AddTimer("SpinnersDownChain1", 24);	
			
			self->AddTimer("FinalEnemies", 16.5f);				
			
			self->AddTimer("UnstunEnemies", 30);		
		}
	}
...
//		Timing for large group final wave audio		
		if (WaveNum == 2) {
			...
			if (LargeGroup == 1) {
				self->AddTimer("MusicOff", 6);
				self->AddTimer("FinalWaveLargeGUID", 10.7f);	
			} else {
				self->AddTimer("MusicOff", 11);
				self->AddTimer("FinalWaveSmallGUID", 18);
			}	
		} else if (WaveNum == 3) {
...
//		Communicate ready to door -> then door communicate ready to spinner		
		auto WavesDoor = Game::entityManager->GetEntitiesInGroup("WavesDoor");	
        for (auto* door : WavesDoor) {		
			door->SetVar(u"bActive", true);
		}	
	...
	} else if (timerName == "FinalWaveLargeGUID") {

		self->SetNetworkVar(u"wave3large", 1);
		self->AddTimer("FinalWaveMusic", 34.2);	 	
	} else if (timerName == "FinalWaveSmallGUID") {

		self->SetNetworkVar(u"wave3small", 1);	
		self->AddTimer("FinalWaveMusic", 10.2);	 		
