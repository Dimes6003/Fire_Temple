...
void SeeSaw09::OnStartup(Entity* self) {
...
void SeeSaw09::OnSkillEventFired(Entity* self, Entity* caster, const std::string& message) {
...
void SeeSaw09::TriggerDrill(Entity* self) {
//	Move spinner first
...
// 	End		
	...
	self->CancelTimer("MoveUp");
	self->CancelTimer("MoveUp");
	self->CancelTimer("MoveUp");
}

void SeeSaw09::SpawnLegs(Entity* self, const std::string& loc) {
...
void SeeSaw09::OnChildLoaded(Entity* self, Entity* child) {
...
void SeeSaw09::NotifyDie(Entity* self, Entity* other, Entity* killer) {
...
void SeeSaw09::OnChildRemoved(Entity* self, Entity* child) {
	RenderComponent::PlayAnimation(self, u"down");	
	self->AddTimer("DownAnim", 0.1f);	
	auto legTable = self->GetVar<std::vector<LWOOBJID>>(u"legTable");
...
void SeeSaw09::OnWaypointReached(Entity* self, uint32_t waypointIndex) {
	if (waypointIndex == 0) {
		self->SetNetworkVar(u"bIsInUse", false);
		self->SetVar(u"bActive", true);
	}
...
void SeeSaw09::OnTimerDone(Entity* self, std::string timerName) {
