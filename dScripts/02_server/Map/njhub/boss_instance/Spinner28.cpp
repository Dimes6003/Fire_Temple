...
void Spinner28::OnStartup(Entity* self) {
	...
	self->AddTimer("SkillPulse", 0.9f);

	self->SetProximityRadius(5.9, "damage_distance");	
	
	self->AddTimer("BladeGUID", 11.1f);
}

void Spinner28::OnSkillEventFired(Entity* self, Entity* caster, const std::string& message) {
...
void Spinner28::TriggerDrill(Entity* self) {
...
void Spinner28::SpawnLegs(Entity* self, const std::string& loc) {
...
// Change in hf now
// if spinner up to down: LegLocY = Spinner starting pos (aka Waypoint 1) - 0.2
...
void Spinner28::OnChildLoaded(Entity* self, Entity* child) {
...
void Spinner28::NotifyDie(Entity* self, Entity* other, Entity* killer) {
...
void Spinner28::OnChildRemoved(Entity* self, Entity* child) {
	RenderComponent::PlayAnimation(self, u"down");	
...
//		Descend sfx
		GameMessages::SendStopNDAudioEmitter(self, self->GetSystemAddress(), "{0f7720ff-d048-421c-87a5-b907c10a5d20}");		
...
void Spinner28::OnProximityUpdate(Entity* self, Entity* entering, std::string name, std::string status) {
...
void Spinner28::OnTimerDone(Entity* self, std::string timerName) {
...
//Handle spinner sound orders
	...
	if (timerName == "BladeGUID") {
		GameMessages::SendPlayNDAudioEmitter(self, self->GetSystemAddress(), "{0f7720ff-d048-421c-87a5-b907c10a5d20}");	
