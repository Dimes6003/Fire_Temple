...
void Spinner33::OnStartup(Entity* self) {
	m_Counter = 1;		
...
//	Blade sfx
	self->AddTimer("BladeGUID", 11.4f);		
	
}

void Spinner33::SpawnLegs(Entity* self, const std::string& loc) {
...
void Spinner33::OnChildLoaded(Entity* self, Entity* child) {
...
void Spinner33::NotifyDie(Entity* self, Entity* other, Entity* killer) {
...
void Spinner33::OnChildRemoved(Entity* self, Entity* child) {
	RenderComponent::PlayAnimation(self, u"down");	
	...
	if (legTable.empty()) {	
		...
		self->AddTimer("ReturnIdle", 5.5f);
		self->AddTimer("RespawnLeg", 6.5f);
		
//		Descend sfx
		GameMessages::SendStopNDAudioEmitter(self, self->GetSystemAddress(), "{3428fd62-4174-4e6b-b197-393b27f21bb4}");		
...
void Spinner33::OnProximityUpdate(Entity* self, Entity* entering, std::string name, std::string status) {
...
void Spinner33::OnTimerDone(Entity* self, std::string timerName) {
...
//Handle spinner sound orders
	...
	if (timerName == "BladeGUID") {
		...
		if (AttachedPath == u"ZSpinner33") {
			GameMessages::SendPlayNDAudioEmitter(self, self->GetSystemAddress(), "{3428fd62-4174-4e6b-b197-393b27f21bb4}");
		} else { 
			GameMessages::SendPlayNDAudioEmitter(self, self->GetSystemAddress(), "{98dea9a6-ab2b-49f5-8676-2133330d6c61}");		
		}	
